...
class TestTaskParamsService(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_patch_attr_delegates_to_repository_update_attr(self):
        item_id = uuid4()
        expected = {"id": str(item_id), "value": "normal"}
        self.task_params_repository.update_attr.return_value = expected

        result = await self.service.patch_attr(item_id, "value", "normal")
    ...
    async def test_put_update_delegates_to_repository_whole_update(self):
        item_id = uuid4()
        ...
        result = await self.service.put_update(item_id, schema)
    ...
    async def test_remove_by_id_delegates_to_repository_delete_by_id(self):
        item_id = uuid4()
        self.task_params_repository.delete_by_id.return_value = None

        result = await self.service.remove_by_id(item_id)
