...
@pytest.mark.asyncio
async def test_get_user_config_returns_response_when_config_exists():
    repository = AsyncMock()
    config = _build_config()
    repository.get_by_user_and_game.return_value = config
    service = UserGameConfigService(repository)

    result = await service.get_user_config(config.userId, config.gameId)
...
@pytest.mark.asyncio
async def test_get_user_config_returns_none_when_missing():
    repository = AsyncMock()
    repository.get_by_user_and_game.return_value = None
    service = UserGameConfigService(repository)

    result = await service.get_user_config("user-id", "game-id")

    assert result is None


@pytest.mark.asyncio
async def test_create_user_config_calls_repository_and_returns_response():
    repository = AsyncMock()
    config = _build_config(
    ...
    result = await service.create_user_config(schema)
...
@pytest.mark.asyncio
async def test_update_user_config_returns_none_when_existing_config_not_found():
    repository = AsyncMock()
    repository.get_by_user_and_game.return_value = None
    service = UserGameConfigService(repository)
...
@pytest.mark.asyncio
async def test_delete_user_config_delegates_to_repository():
    repository = AsyncMock()
    repository.delete.return_value = True
    service = UserGameConfigService(repository)

    result = await service.delete_user_config("user-id", "game-id")
