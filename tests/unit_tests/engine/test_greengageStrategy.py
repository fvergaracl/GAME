...
def strategy_bundle():
    ...
    with patch(
        ...
        "app.engine.greengageStrategy.Container.user_points_analytics_service",
        new=MagicMock(return_value=user_points_analytics_service),
    ):
        strategy = GREENGAGEGamificationStrategy()
    strategy.debug = False
    return strategy, task_service, user_points_service, user_points_analytics_service
...
def test_get_dpte_uses_floor_time_range_and_fallback(strategy_bundle):
    strategy, _, _, _ = strategy_bundle
...
async def test_calculate_points_case_1_1_and_maps_task_params(strategy_bundle):
    ...
    user_points_analytics_service.count_personal_records_by_external_game_id.assert_not_called()
...
async def test_calculate_points_case_3_when_minutes_are_above_global_average(
    ...
    user_points_analytics_service.get_personal_avg_by_external_game_id.assert_not_called()
