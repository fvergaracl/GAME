...
from app.schema.games_schema import (
    BaseGame, BaseGameResult, PostCreateGame, PatchGame, GameCreated,
    ResponsePatchGame, FindGameById, PostFindGame, FindGameResult,
    ListTasksWithUsers
)
...
def test_base_game_result():
    ...
    data = {
        ...
        "platform": "PC",
        "params": [
            BaseFindGameParams(id=uuid4(), key="difficulty", value="hard")
        ]
    }
...
def test_post_create_game():
    ...
    data = {
        ...
        "strategyId": "strategy123",
        "params": [
            CreateGameParams(key="difficulty", value="hard")
        ]
    }
...
def test_patch_game():
    ...
    data = {
        ...
        "platform": "PC",
        "params": [
            UpdateGameParams(id=uuid4(), key="difficulty", value="hard")
        ]
    }
    game = PatchGame(**data)
    assert game.externalGameId == data["externalGameId"]
    ...
    data = {
        "items": [
            BaseGameResult(
                ...
                params=[
                    BaseFindGameParams(
                        id=uuid4(), key="difficulty", value="hard")
                ]
            )
        ],
        "search_options": SearchOptions(ordering="asc",
                                        page=1, page_size=10,
                                        total_count=1)
    }
