...
from app.schema.apikey_schema import (
    ApiKeyCreated, ApiKeyPostBody, ApiKeyCreate, ApiKeyCreatedUnitList
)
...
async def create_api_key(
    ...
    apikeyBody = ApiKeyCreate(
        **schema.dict(), createdBy=created_by, apiKey=apiKey)
    response = service.create_api_key(apikeyBody)

    return ApiKeyCreated(
        **response.dict(), message="API Key created successfully"
    )
