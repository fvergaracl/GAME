    ...
    **Note**: Strategies must extend a base strategy class and implement all
    ...
    for file in os.listdir("app/engine"):
        if (
            ...
            and file != "base_strategy.py"
            and file != "check_base_strategy_class.py"
            and file != "all_engine_strategies.py"
        ):
            strategy = file[:-3]
    ...
    for strategy in strategies:
        ...
        classes = [getattr(module, name) for name in dir(module) if name[0].isupper()]
    ...
    all_strategies_classes = list(
        filter(lambda x: x.get_strategy_id() != "BaseStrategy", all_strategies_classes)
    )
